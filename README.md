...
├── .env.example             # пример файла с переменными окружения для настройки проекта
├── .gitignore               # список файлов и папок, которые не должны попадать в GitHub
└── README.md                # основная документация проекта: описание, запуск, API, безопасность и планы развития
```
